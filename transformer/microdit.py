...
class MicroDiT(nn.Module):
    ...
    def __init__(
        ...
        timestep_caption_embed_dim: int | None = None,
        pos_embed_dim: int | None = None,
        num_experts: int = 4,
        active_experts: int = 2,
        dropout: float = 0.1,
        patch_mixer_layers: int = 2,
        ...
        if self.embed_cat:
            self.pos_embed_dim = (
                pos_embed_dim if pos_embed_dim is not None else embed_dim
            )
            self.timestep_caption_embed_dim = (
                timestep_caption_embed_dim
                if timestep_caption_embed_dim is not None
                else embed_dim
            )
            self.total_embed_dim = (
                self.timestep_caption_embed_dim + self.pos_embed_dim + embed_dim
            )
        else:
        ...
        self.mha = nn.MultiheadAttention(
            self.timestep_caption_embed_dim, num_heads, batch_first=True
        )
        ...
        self.linear = nn.Linear(
            self.timestep_caption_embed_dim, self.timestep_caption_embed_dim
        )

        # Patch-mixer
        self.patch_mixer = PatchMixer(
            self.total_embed_dim, num_heads, patch_mixer_layers
        )
    ...
    def forward(
        ...
        pos_embed = get_2d_sincos_pos_embed(
            self.pos_embed_dim, height // patch_size_h, width // patch_size_w
        )
        ...
        # Timestep embedding
        t_emb = self.time_embed.forward(t)  # (batch_size, timestep_caption_embed_dim)

        # Caption embedding
        c_emb = self.caption_embed.forward(
            caption_embeddings
        )  # (batch_size, timestep_caption_embed_dim)

        mha_out = self.mha.forward(
            t_emb.unsqueeze(1), c_emb.unsqueeze(1), c_emb.unsqueeze(1)
        )[0].squeeze(1)
        mlp_out = self.mlp.forward(mha_out)

        # Pool + MLP
        pool_out = self.pool_mlp.forward(mlp_out.unsqueeze(2))

        # Pool + MLP + t_emb
        pool_out = (pool_out + t_emb).unsqueeze(1)

        # Apply linear layer
        cond_signal = self.linear.forward(mlp_out).unsqueeze(
            1
        )  # (batch_size, 1, timestep_caption_embed_dim)
        ...
        if self.embed_cat:
            x[:, :, -self.timestep_caption_embed_dim :] = (
                x[:, :, -self.timestep_caption_embed_dim :] + cond
            )
        else:
            x = x + cond

        # Backbone transformer model
        x = self.backbone.forward(x, c_emb)
...
class CustomDataset(IterableDataset[CustomDatasetType]):
    ...
    def __iter__(self):
        ...
        for batch_data, resolution in self.bucket_manager.generator():
            ...
            indices = [
                self.index_image_id_map[f"{int(image_id):08d}"]
            ...
            for item in batch_items:
                ...
                latent = torch.tensor(item["latent"]).reshape(
                    4, resolution[0], resolution[1]
                )
...
class LitMicroDiT(L.LightningModule):
    def __init__(
        ...
        dataset_path: str = f"{DS_DIR_BASE}/{DATASET_NAME}",
        bucket_file: str = f"{DS_DIR_BASE}/{METADATA_DATASET_NAME}/res_map.json",
        index_image_id_map_path: str = f"{DS_DIR_BASE}/{METADATA_DATASET_NAME}/image_id_map.json",
    ):
        super().__init__()
        self.model = model
    ...
    def configure_optimizers(self):
        optimizer = torch.optim.adamw.AdamW(self.parameters(), lr=self.learning_rate)
    ...
    def forward(self, x: torch.Tensor, t: torch.Tensor, mask: torch.Tensor) -> torch.Tensor
        return self.model.forward(x, t, mask)
    ...
    def training_step(self, batch: CustomDatasetType, batch_idx: int) -> torch.Tensor:
